...
public class CANDeviceFinder {
  ...
  class APIFinder extends CanFinder {
    ...
    void report() {
      for (int id : idsPresent) {
        ...
        // logger.info ("{}: API {} {} = msg {}", canDeviceType, String.format("%2d", apiId),
        // String.format("0x%03X", apiId), String.format("%08X", id));
      }
    }
  }
  ...
  */
  ...
  private int canBusId(int devType, int mfg, int apiId, int devId) {
    ...
    int rv =
        ((devType & 0xff) << 24) | ((mfg & 0xff) << 16) | ((apiId & 0x3ff) << 6) | (devId & 0x3f);
    if (logCanBusIds) {
